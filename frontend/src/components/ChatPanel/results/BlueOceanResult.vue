...
<!-- ROI tooltip 内容挂载于 body，需非 scoped 全局样式 -->
<style>
.roi-tip {
  min-width: 190px;
}
...
.roi-tip-row.total b.roi-high {
  color: #52c41a;
}
.roi-tip-row.total b.roi-mid {
  color: #ffec3d;
}
.roi-tip-row.total b.roi-low {
  color: #ff7875;
}

/* ===== 详情抽屉（挂载于 body，需非 scoped 全局样式） ===== */
.detail-hero {
  display: flex;
  gap: 12px;
...
.dkpi-val.roi-high { color: #52c41a; }
.dkpi-val.roi-mid { color: #d48806; }
.dkpi-val.roi-low { color: #cf1322; }
...
.drow b.good { color: #389e0d; }
.drow b.warn { color: #fa541c; }
.trend-up { color: #389e0d !important; }
.trend-down { color: #cf1322 !important; }
.muted { color: #bfbfbf !important; font-weight: 400; }
...
.cost-row span { color: #8c8c8c; }
.cost-row b { color: #262626; font-variant-numeric: tabular-nums; }
.cost-row.net b { color: #d48806; }
.cost-row.total { background: rgba(250, 140, 22, 0.1); }
.cost-row.total b.roi-high { color: #52c41a; }
.cost-row.total b.roi-mid { color: #d48806; }
.cost-row.total b.roi-low { color: #cf1322; }
.cost-divider {
  height: 1px;
...
.detail-actions .mon-btn {
  margin-top: 8px;
}

/* ====== 深色模式精调 ======
   Header: 暗紫渐变提亮 + 微光晕，避免"黑斑"感
   统计卡: 加左侧品牌色竖条 + 背景提亮 + 微内阴影，增加层次 */
:global(html.dark) .result-header {
...
:global(html.dark) .stat-card::before {
  content: '';
  position: absolute;
...
:global(html.dark) .stat-excellent {
  background: rgba(34, 197, 94, 0.12);
}
:global(html.dark) .stat-excellent::before { background: #22c55e; }
:global(html.dark) .stat-excellent .stat-value { color: #4ade80; }

:global(html.dark) .stat-medium {
  background: rgba(234, 179, 8, 0.12);
}
:global(html.dark) .stat-medium::before { background: #eab308; }
:global(html.dark) .stat-medium .stat-value { color: #facc15; }

:global(html.dark) .stat-poor {
  background: rgba(239, 68, 68, 0.10);
}
:global(html.dark) .stat-poor::before { background: #ef4444; }
:global(html.dark) .stat-poor .stat-value { color: #f87171; }

:global(html.dark) .stat-saved {
  background: rgba(59, 130, 246, 0.12);
}
:global(html.dark) .stat-saved::before { background: #3b82f6; }
:global(html.dark) .stat-saved .stat-value { color: #60a5fa; }

:global(html.dark) .stat-label {
  color: var(--text-tertiary);
}
:global(html.dark) .stats-row {
  border-bottom-color: var(--border-base);
}
:global(html.dark) .blue-ocean-result {
